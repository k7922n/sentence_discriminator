...
def form_vocab_mapping(filename, max_size):
  
  output_path = filename + str(max_size) + '.mapping'
  
  if gfile.Exists(output_path):
    print('Mapping of %s has already been formed!' % filename)
  else:
    print('Forming mapping file according to %s' % filename)  
    ...
    with gfile.GFile(filename, mode = 'rb') as f:
      ...
      vocab_list = [_UNK] + sorted(vocab, key = vocab.get, reverse = True)
...
def prepare_whole_data(input_path, max_size):
  form_vocab_mapping(input_path, max_size)
  map_path = input_path + str(max_size) + '.mapping'  
  vocab_map = read_map(map_path)
  file_to_token(input_path, vocab_map)
